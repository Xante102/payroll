...
router.get("/", (req, res) => {
  ...
  // const employees = `SELECT emp.id, log.id as login_id, emp.f_name,emp.l_name, log.email, log.password, rl.id AS role_id, dpt.id AS dpt_id, dpt.department_name, rl.roles FROM payroll3.employees emp JOIN payroll3.logins log ON log.emp_id = emp.id JOIN payroll3.departments dpt ON log.dpt_id = dpt.id JOIN payroll3.roles rl ON log.role_id = rl.id`;
  conn.query(employees, (err, eRows) => {
    if (err) throw err;
    res.render("employees/employee-list", {
...
router.get("/edit/:id", (req, res) => {
  ...
  const editSql = `SELECT emp.id, log.id AS login_id, emp.f_name, emp.l_name, log.email, log.password, rl.id AS role_id, dpt.id AS dpt_id, dpt.department_name, rl.roles FROM payroll3.employees emp JOIN payroll3.logins log ON log.emp_id = emp.id JOIN payroll3.departments dpt ON log.dpt_id = dpt.id JOIN payroll3.roles rl ON log.role_id = rl.id WHERE emp.id = ${id}`;
  const roles = `SELECT * FROM roles`;
  const departments = `SELECT * FROM departments`;
...
router.post("/update", (req, res) => {
  const updateEmp = `UPDATE employees SET dpt_id='${req.body.dpt_id}', f_name='${req.body.f_name}', l_name='${req.body.l_name}', where id =${req.body.id}`;
